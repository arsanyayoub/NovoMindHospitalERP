import{P as s}from"./chunk-VSY2NHHR.js";var h=(()=>{class c{toCSV(t,n,o){if(!t?.length)return;let e=o||Object.keys(t[0]).map(r=>({key:r,header:r})),l=e.map(r=>`"${r.header}"`).join(","),i=t.map(r=>e.map(d=>{let p=this.getNestedValue(r,d.key);return`"${String(p??"").replace(/"/g,'""')}"`}).join(",")),a=[l,...i].join(`\r
