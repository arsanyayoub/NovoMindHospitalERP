    ...
    <div class="p-8 bg-[#f8fafc] min-h-screen">
      <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-end mb-10">
          <div>
            <h1 class="text-4xl font-black text-slate-900 tracking-tight mb-2">Health <span class="text-indigo-600">{{ 'UI_EXCHANGE' | translate }}</span> & Referrals</h1>
            <p class="text-slate-500 font-medium">{{ 'UI_INTEROPERABILITY_AND_EXTERNA' | translate }}</p>
          </div>
    <app-module-dashboard class="no-print" name="referrals"></app-module-dashboard>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
           ...
           <div class="lg:col-span-8 space-y-8">
              <div class="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
                 <div class="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                    <h3 class="font-black text-lg text-slate-800 uppercase tracking-tight">{{ 'UI_ACTIVE_REFERRALS' | translate }}</h3>
                    <div class="flex gap-2">
                       <span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase">{{ 'UI_OUTBOUND' | translate }}</span>
                    </div>
                 </div>
                 <div class="divide-y divide-slate-50">
                    <div *ngFor="let r of referrals" class="p-8 hover:bg-slate-50/50 transition-all flex items-center justify-between group">
                       ...
                       <div class="text-center hidden md:block">
                          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{{ 'UI_STATUS' | translate }}</p>
           ...
           <div class="lg:col-span-4 space-y-6">
              <div class="bg-indigo-900 rounded-[40px] p-8 text-white shadow-2xl relative overflow-hidden">
                 ...
                 <h4 class="text-xs font-black text-indigo-400 uppercase tracking-widest mb-8">{{ 'UI_REGISTERED_FACILITIES' | translate }}</h4>
                 <div class="space-y-4">
              ...
              <div class="bg-white rounded-[40px] p-8 border border-slate-100 shadow-sm">
                 <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-8">{{ 'UI_HIE_HEALTH' | translate }}</h4>
                 <div class="space-y-6">
                    <div>
                       <div class="flex justify-between text-[10px] font-black uppercase mb-2">
                          <span class="text-emerald-600">{{ 'UI_SYNC_STATUS' | translate }}</span>
                    ...
                    <div class="flex gap-4 p-4 bg-slate-50 rounded-2xl">
                       ...
                       <div>
                          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ 'UI_ACTIVE_STANDARD' | translate }}</p>
                          <p class="font-black text-slate-800 text-sm">{{ 'UI_HL7_FHIR_V4_0_1' | translate }}</p>
                       </div>
                    </div>
                 </div>
              </div>
      ...
      <div *ngIf="showCreate" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
         <div class="bg-white rounded-[40px] shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-300">
            <div class="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
               <h3 class="font-black text-2xl text-slate-800 tracking-tight">Create <span class="text-indigo-600">{{ 'UI_REFERRAL' | translate }}</span></h3>
            ...
            <div class="p-10 space-y-6">
               <div class="grid grid-cols-2 gap-6">
                  <div>
                     <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{{ 'UI_TARGET_FACILITY' | translate }}</label>
                  ...
                  <div>
                     <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{{ 'UI_REFERRAL_TYPE' | translate }}</label>
                     <select [(ngModel)]="newReferral.referralType" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-semibold">
                        <option value="Outbound">{{ 'UI_OUTBOUND_TRANSFER' | translate }}</option>
                        <option value="Inbound">{{ 'UI_INBOUND_ACCEPT' | translate }}</option>
                     </select>
                  </div>
               </div>
               <div>
                  <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{{ 'UI_PATIENT_CODE_ID' | translate }}</label>
                  <input type="text" [(ngModel)]="newReferral.patientId" [placeholder]="'UI_ENTER_PATIENT_ID' | translate" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-semibold uppercase">
               </div>
               <div>
                  <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{{ 'UI_MEDICAL_REASON' | translate }}</label>
                  <textarea [(ngModel)]="newReferral.reason" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-semibold placeholder:text-slate-300" [placeholder]="'UI_DESCRIBE_THE_CLINICAL_NECESS' | translate"></textarea>
               </div>
            </div>
            <div class="p-10 bg-slate-50/50 flex gap-4">
               <button (click)="showCreate = false" class="flex-1 px-8 py-4 border-2 border-slate-200 rounded-2xl font-black text-slate-500 hover:bg-white active:scale-95 transition-all uppercase tracking-widest text-[10px]">{{ 'UI_CANCEL' | translate }}</button>
               <button (click)="saveReferral()" class="flex-1 px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black hover:bg-indigo-700 shadow-xl shadow-indigo-100 active:scale-95 transition-all uppercase tracking-widest text-[10px]">{{ 'UI_SUBMIT_REFERRAL' | translate }}</button>
            </div>
         </div>
      </div>
      ...
      <div *ngIf="showPayload" class="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[60] flex items-center justify-center p-8">
         <div class="bg-slate-900 border border-white/10 rounded-[40px] shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col overflow-hidden">
            <div class="p-8 border-b border-white/5 flex justify-between items-center bg-white/5">
                <div>
                  <h3 class="text-white font-black text-xl leading-tight">FHIR <span class="text-emerald-400">{{ 'UI_BUNDLE' | translate }}</span></h3>
                  <p class="text-white/40 text-[10px] font-black uppercase tracking-widest mt-1">{{ 'UI_EXPORTED_STANDARD_COMPLIANT_' | translate }}</p>
                </div>
