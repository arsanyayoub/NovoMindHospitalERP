...
@Component({
    ...
    <div class="p-6 bg-slate-50 min-h-screen">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight">Mortuary <span class="text-slate-400">{{ 'UI_MANAGEMENT' | translate }}</span></h1>
          <p class="text-slate-500 font-medium italic">{{ 'UI_SECURE_AND_DIGNIFIED_DECEASE' | translate }}</p>
        </div>
    <app-module-dashboard class="no-print" name="mortuary"></app-module-dashboard>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-10">
        <div class="lg:col-span-1 space-y-6">
           ...
           <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">{{ 'UI_CURRENT_STATUS' | translate }}</h4>
              <div class="space-y-6">
                <div>
                   <p class="text-3xl font-black text-slate-800">{{records.length}}</p>
                   <p class="text-xs font-bold text-slate-400 uppercase">{{ 'UI_IN_CUSTODY' | translate }}</p>
                </div>
                <div>
                   <p class="text-3xl font-black text-emerald-600">85%</p>
                   <p class="text-xs font-bold text-slate-400 uppercase">{{ 'UI_CAPACITY_USED' | translate }}</p>
                </div>
                <div class="pt-6 border-t border-slate-50">
                   <p class="text-sm font-bold text-slate-600 mb-2">{{ 'UI_CHAMBER_AVAILABILITY' | translate }}</p>
           ...
           <div class="bg-indigo-600 rounded-3xl p-8 text-white shadow-xl">
              <h4 class="font-bold mb-4">{{ 'UI_REGULATORY_TASKS' | translate }}</h4>
              ...
              <table class="w-full text-left">
                <thead>
                   <tr class="bg-slate-50 border-b border-slate-100">
                     <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase">{{ 'UI_DECEASED_NAME' | translate }}</th>
                     <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase">{{ 'UI_STATUS' | translate }}</th>
                     <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase">{{ 'UI_LOCATION' | translate }}</th>
                     <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase">{{ 'UI_RECEIVED_DATE' | translate }}</th>
                     <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase text-right">{{ 'UI_ACTIONS' | translate }}</th>
                   </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                   ...
                   <tr *ngIf="!records.length">
                      <td colspan="5" class="py-20 text-center text-slate-300 font-bold italic">{{ 'UI_NO_RECORDS_CURRENTLY_IN_REGI' | translate }}</td>
                   </tr>
                </tbody>
              </table>
           </div>
      ...
      <div *ngIf="showNewRecord" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
         <div class="bg-white rounded-[40px] shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-300">
            <div class="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
               <div>
                  <h3 class="font-black text-2xl text-slate-800 tracking-tighter">Deceased <span class="text-slate-400">{{ 'UI_ENTRY' | translate }}</span></h3>
                  <p class="text-xs font-bold text-slate-400 uppercase mt-1">{{ 'UI_OFFICIAL_MORTALITY_REGISTRY' | translate }}</p>
               </div>
            ...
            <div class="p-10 space-y-8">
               <div class="grid grid-cols-2 gap-6">
                  <div class="col-span-2">
                     <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ 'UI_SUBJECT_NAME_PATIENT_ID' | translate }}</label>
                  ...
                  <div>
                     <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ 'UI_D_O_D' | translate }}</label>
                  ...
                  <div>
                     <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ 'UI_CHAMBER_LOCATION' | translate }}</label>
                     <input type="text" [(ngModel)]="newRecord.chamberNumber" [placeholder]="'UI_EXP_C_14' | translate" class="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none focus:border-slate-800 transition-all font-bold text-slate-800 uppercase">
                  </div>
                  <div class="col-span-2">
                     <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ 'UI_CERTIFIED_CAUSE_OF_DEATH' | translate }}</label>
            ...
            <div class="p-10 bg-slate-50/50 flex gap-4">
               <button (click)="showNewRecord = false" class="flex-1 px-8 py-4 border-2 border-slate-200 rounded-2xl font-black text-slate-500 hover:bg-white transition-all active:scale-95">{{ 'UI_DISCARD' | translate }}</button>
               <button (click)="saveRecord()" class="flex-1 px-8 py-4 bg-slate-800 text-white rounded-2xl font-black hover:bg-slate-900 shadow-xl shadow-slate-200 transition-all active:scale-95">{{ 'UI_FINALIZE_ENTRY' | translate }}</button>
            </div>
         </div>
      </div>
    </div>
