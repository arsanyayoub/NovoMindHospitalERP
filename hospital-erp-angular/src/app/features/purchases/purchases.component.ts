...
@Component({
  ...
  template: `
    <div class="page-header">
      <div><h1 class="page-title">{{ 'PURCHASES' | translate }}</h1></div>
      <div class="flex gap-2">
        <button class="btn" [class.btn-primary]="tab==='suppliers'" [class.btn-secondary]="tab!=='suppliers'" (click)="tab='suppliers'">{{ 'SUPPLIERS' | translate }}</button>
        <button class="btn" [class.btn-primary]="tab==='invoices'" [class.btn-secondary]="tab!=='invoices'" (click)="tab='invoices';loadInvoices()">{{ 'PURCHASE_INVOICES' | translate }}</button>
      </div>
    </div>

    <!-- Suppliers Tab -->
    <div *ngIf="tab==='suppliers'">
      <div class="flex justify-between items-center mb-4">
        <div class="search-bar"><span class="material-icons-round search-icon">search</span><input class="form-control" [placeholder]="'SEARCH' | translate" [(ngModel)]="search" (input)="loadSuppliers()"></div>
        <button class="btn btn-primary btn-sm" (click)="openSupplierForm()"><span class="material-icons-round">add</span> {{ 'ADD_NEW' | translate }}</button>
      </div>
      <div class="card" style="padding:0"><div class="table-container"><table class="table">
        ...
        <tbody><tr *ngFor="let s of suppliers">
          <td><span class="badge badge-primary">{{ s.supplierCode }}</span></td>
          <td class="font-semibold">{{ s.supplierName }}</td>
          <td>{{ s.contactPerson }}</td>
          <td>{{ s.phoneNumber }}</td>
          <td [class.text-danger]="s.balance>0" [class.text-success]="s.balance<=0">{{ s.balance | currency }}</td>
          <td><div class="flex gap-1">
            <button class="btn btn-sm btn-secondary" (click)="editSup(s)"><span class="material-icons-round" style="font-size:16px">edit</span></button>
          </div></td>
        </tr></tbody>
      </table></div></div>
    </div>
    ...
    <div *ngIf="tab==='invoices'">
      ...
      <div class="card" style="padding:0"><div class="table-container"><table class="table">
        <thead><tr><th>Invoice #</th><th>Supplier</th><th>Date</th><th>Due Date</th><th>Total</th><th>Paid</th><th>Remaining</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody><tr *ngFor="let i of invoices">
          <td><span class="badge badge-info">{{ i.invoiceNumber }}</span></td>
          <td class="font-semibold">{{ i.supplierName }}</td>
    ...
    <!-- Supplier Modal -->
    <div class="modal-overlay" *ngIf="showSupForm" (click)="showSupForm=false"><div class="modal" (click)="$event.stopPropagation()">
      <div class="modal-header"><h3>{{ supForm.id ? 'Edit' : 'New' }} Supplier</h3><button class="btn btn-sm btn-secondary" (click)="showSupForm=false"><span class="material-icons-round">close</span></button></div>
      <div class="modal-body">
        <div class="form-row"><div class="form-group"><label class="form-label">Name *</label><input class="form-control" [(ngModel)]="supForm.supplierName"></div><div class="form-group"><label class="form-label">Contact Person</label><input class="form-control" [(ngModel)]="supForm.contactPerson"></div></div>
        <div class="form-row mt-4"><div class="form-group"><label class="form-label">Phone</label><input class="form-control" [(ngModel)]="supForm.phoneNumber"></div><div class="form-group"><label class="form-label">Email</label><input class="form-control" [(ngModel)]="supForm.email"></div></div>
        <div class="form-row mt-4"><div class="form-group"><label class="form-label">Address</label><input class="form-control" [(ngModel)]="supForm.address"></div><div class="form-group"><label class="form-label">Tax Number</label><input class="form-control" [(ngModel)]="supForm.taxNumber"></div></div>
      </div>
      <div class="modal-footer"><button class="btn btn-secondary" (click)="showSupForm=false">{{ 'CANCEL' | translate }}</button><button class="btn btn-primary" (click)="saveSup()">{{ 'SAVE' | translate }}</button></div>
    </div></div>
    ...
    <div class="modal-overlay" *ngIf="showPayModal" (click)="showPayModal=false"><div class="modal" style="max-width:420px" (click)="$event.stopPropagation()">
      ...
      <div class="modal-body">
        <div class="pay-info-card">
          ...
          <div class="pay-row"><span class="text-muted">Supplier</span><span>{{ selectedInvoice?.supplierName }}</span></div>
...
export class PurchasesComponent implements OnInit {
  ...
  loadInvoices() { this.svc.getPurchaseInvoices({}, this.invoiceFilter || undefined).subscribe(r => this.invoices = r.items); }

  saveSup() {
    const obs = this.supForm.id ? this.svc.updateSupplier(this.supForm.id, this.supForm) : this.svc.createSupplier(this.supForm);
    obs.subscribe({ next: () => { this.toast.success('Saved'); this.showSupForm = false; this.loadSuppliers(); }, error: () => this.toast.error('Error saving supplier') });
  }
  ...
  saveInvoice() {
    ...
    this.svc.createPurchaseInvoice(payload).subscribe({
      ...
      error: (e: any) => { this.toast.error(e.error?.message || 'Error'); this.saving = false; }
    });
  }
